...
import { getFavoriteAudioPlayers } from 'apiSdk/favorite-audio-players';
import { FavoriteAudioPlayersInterface } from 'interfaces/favorite-audio-players';
import { getFavoriteSongs } from 'apiSdk/favorite-songs';
...
function DashboardPage() {
  ...
  const favoriteAudioPlayersStat = useSWR<PaginatedInterface<FavoriteAudioPlayersInterface>>(
    () => `/favorite-audio-players`,
    () => getFavoriteAudioPlayers({ limit: 1, offset: 0 }),
  );

  const favoriteSongsStat = useSWR<PaginatedInterface<FavoriteSongsInterface>>(
    () => `/favorite-songs`,
    () => getFavoriteSongs({ limit: 1, offset: 0 }),
  );

  const audioPlayerStat = useSWR<PaginatedInterface<AudioPlayerInterface>>(
    () => `/audio-players`,
    () => getAudioPlayers({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Favorite Audio Players',
      result: favoriteAudioPlayersStat?.data?.totalCount,
      isLoading: favoriteAudioPlayersStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Favorite Songs',
      result: favoriteSongsStat?.data?.totalCount,
      isLoading: favoriteSongsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Audio Players',
      result: audioPlayerStat?.data?.totalCount,
      isLoading: audioPlayerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
