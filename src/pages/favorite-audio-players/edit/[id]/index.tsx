...
import { getFavoriteAudioPlayersById, updateFavoriteAudioPlayersById } from 'apiSdk/favorite-audio-players';
...
function FavoriteAudioPlayersEditPage() {
  ...
  const handleSubmit = async (values: FavoriteAudioPlayersInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateFavoriteAudioPlayersById(id, values);
      mutate(updated);
      resetForm();
      ...
      breadcrumbs={
        <Breadcrumbs
          items={[
            ...
            {
              label: 'Update Favorite Audio Players',
              isCurrent: true,
            },
          ]}
        />
